...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/04/01 20:03:27 by mhonchar          #+#    #+#             */
/*   Updated: 2019/04/01 20:06:21 by mhonchar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ft_set_default(win);
	win->ft_putfract = ft_pthread_spinner;
}

void			ft_draw_spinner(t_win *win, double x, double y)
{
	int		n;
	double	z_re;
	double	z_im;
...
void			ft_pthread_spinner(t_win *win)
{
	...
	while (++i < THREADS_AMOUNT)
		pthread_create((pt + i), NULL, ft_spinner, (void*)(w + i));
	i = -1;
	while (++i < THREADS_AMOUNT)
