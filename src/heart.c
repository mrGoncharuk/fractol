...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/05/09 15:57:37 by mhonchar          #+#    #+#             */
/*   Updated: 2019/05/09 15:58:46 by mhonchar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

void			ft_init_heart(t_win *win)
{
	ft_set_default(win);
	win->ft_putfract = ft_pthread_heart;
}

void			ft_draw_heart(t_win *win, double x, double y)
{
	int		n;
	double	z_re;
	double	z_im;
	...
	{
		z_im = ft_pow(ft_abs(z_re) + z_im, 2) - z_resqr - z_imsqr;
		z_im += y;
...
void			ft_pthread_heart(t_win *win)
{
	...
	while (++i < THREADS_AMOUNT)
		pthread_create((pt + i), NULL, ft_heart, (void*)(w + i));
	i = -1;
	while (++i < THREADS_AMOUNT)
